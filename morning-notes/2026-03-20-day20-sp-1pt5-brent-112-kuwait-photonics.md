---
layout: morning-note
title: "Day 20. S&P −1.5%. Dow −444. Brent $112. Kuwait Refineries on Fire. Photonics Exploding."
headline: "Day 20. S&P −1.5%. Dow −444. Brent $112. Kuwait Refineries on Fire. Photonics Exploding."
date: 2026-03-20
image: /images/notes/2026-03-20.jpg
author: Steven Yuan

snapshot:
  - { label: "S&P 500",          value: "6,506",      dir: down }
  - { label: "Nasdaq",           value: "21,647",     dir: down }
  - { label: "Dow Jones",        value: "45,577",     dir: down }
  - { label: "Russell 2000",     value: "correction", dir: down }
  - { label: "Brent Crude",      value: "$112.19",    dir: up }
  - { label: "COHR",             value: "+15.4%",     dir: up }
  - { label: "LITE",             value: "+12%",       dir: up }
  - { label: "Gold",             value: "$3,305+",    dir: up }

levels:
  - { name: "S&P 500",           value: "6,506",      dir: down }
  - { name: "Nasdaq Composite",  value: "21,647",     dir: down }
  - { name: "Dow Jones",         value: "45,577",     dir: down }
  - { name: "Russell 2000",      value: "correction", dir: down }
  - { name: "WTI Crude",         value: "~$105",      dir: up }
  - { name: "Brent Crude",       value: "$112.19",    dir: up }
  - { name: "VIX",               value: "~31",        dir: up }
  - { name: "10Y Treasury",      value: "~4.45%",     dir: up }
  - { name: "Gold Spot",         value: "$3,305+",    dir: up }

sentiment:
  - { label: "AI Infrastructure",  tone: bullish, pct: 80 }
  - { label: "Energy (E&P)",       tone: bullish, pct: 97 }
  - { label: "Gold / Precious",    tone: bullish, pct: 95 }
  - { label: "Defense",            tone: bullish, pct: 92 }
  - { label: "Photonics / Optics", tone: bullish, pct: 90 }
  - { label: "Equities (broad)",   tone: bearish, pct: 7 }
  - { label: "Consumer / Airlines",tone: bearish, pct: 5 }

tags:
  - oil
  - Brent
  - Iran
  - Kuwait
  - Iraq
  - photonics
  - COHR
  - LITE
  - MU
  - FDX
  - war
  - gold
  - defense
  - correction
---

The market had its worst day since the war began. S&P −1.51% to 6,506, Nasdaq −2.01% to 21,647, Dow −444 to 45,577. Russell 2000 fell 2%+ into official correction territory. Dow and Nasdaq came within inches of correction thresholds intraday. The catalyst: Iraq declared force majeure on all foreign-operated oilfields because it cannot ship crude through Hormuz. Iranian drones struck two Kuwait refineries (Mina Al-Ahmadi and Mina Abdullah, combined 730K bpd) during Eid al-Fitr. Brent hit $112.19, the highest since the 2022 commodity shock. Pentagon officials reportedly made preparations for US ground troops into Iran, which sent the market through the floor. And yet, in the middle of this carnage, one sector is on fire — literally lighting up: optical/photonics. COHR +15.4%, LITE +12%, AAOI surging. OFC 2026 in LA is showcasing AI-scale optics breakthroughs. Both COHR and LITE join the S&P 500 on March 23. NVIDIA's multi-billion-dollar strategic deals with both companies. MU +8% from its blowout. FDX beat and raised. AI infrastructure is immune to the macro, for now. This is a market with two realities. Choose your side.

## Market Snapshot | Thu Close + Friday Pre-Market ~06:45 ET

| Index | Thu Close | Day Chg | YTD | Context |
|---|---|---|---|---|
| **S&P 500** | **6,506** | **−1.51%** | **−9.1%** | Worst day of 2026; nearing correction territory |
| **Nasdaq Composite** | **21,647** | **−2.01%** | **~−10.9%** | In correction; rate + war double compression |
| **Dow Jones** | **45,577** | **−444 pts** | **~−7.2%** | Kuwait refineries + ground troop prep drove the drop |
| **Russell 2000** | correction | **−2.0%+** | **~−13.5%** | Official correction territory; small caps hardest hit |

## Oil, Volatility & Key Instruments

| Asset | Level | Day Chg | Signal |
|---|---|---|---|
| **Brent Crude** | **$112.19** | **+2.1%** | Highest since 2022; Kuwait 730K bpd offline |
| **WTI Crude** | **~$105** | **+2.0%** | Iraq force majeure + Kuwait fire = supply crunch |
| **VIX** | **~31** | **elevated** | Near 2026 highs; ground troop prep = tail risk priced |
| **Gold** | **$3,305+** | **+0.5%** | Structural bid; safe haven + stagflation |
| **10-Yr Yield** | **~4.45%** | **+3 bps** | Inflation > recession fear; curve steepening |

## Photonics Sector | The Light Show in a Dark Market

**Why optics now?** As AI clusters scale beyond single racks, the bottleneck shifts from compute (NVDA solved) to interconnect. Optical interconnects (transceivers, co-packaged optics, circuit switching) are the only way to move data at the speeds AI training requires. COHR and LITE are the picks-and-shovels of this transition.

**NVIDIA validation.** NVDA signed multi-billion-dollar deals with both COHR and LITE at GTC/OFC. When the $1T order book needs optical connectivity, it flows through these two companies. This is demand visibility for 3–5 years.

**S&P 500 inclusion (March 23).** Both COHR and LITE join the index Monday. Index fund buying creates a mechanical demand floor. Combined with the fundamental story, this is a rare alignment of technical and fundamental catalysts.

**MU validates the downstream.** Micron's $33.5B Q3 guide proves HBM demand is accelerating. More HBM = more GPU racks = more optical interconnects. The MU blowout is a leading indicator for COHR/LITE order flow.

| Name | Thu Move | S&P 500 Add | Catalyst |
|---|---|---|---|
| **COHR (Coherent)** | **+15.4%** | March 23 | OFC 2026 + NVDA multi-billion partnership |
| **LITE (Lumentum)** | **+12%** | March 23 | OFC 2026 + NVDA multi-billion partnership |
| **AAOI** | surging | — | OFC 2026 AI optics wave |

## FedEx (FDX) Q3 | Beat, Raised — But the Global Trade Signal Is Mixed

> **📦 FDX Q3 Beat and Raised. But Hormuz Is the Unknown.**
>
> FedEx beat Q3 estimates and raised full-year guidance — a testament to the resilience of the core domestic business. However, the international freight picture is complicated: rerouting cargo around the Cape of Good Hope adds 10–14 days per voyage and significant fuel costs. Management guided conservatively on international volumes for Q4, citing the "ongoing geopolitical uncertainty in the Middle East." The domestic beat is real; the international guidance is the warning shot.

## War Escalation | Day 20 — Iraq FM. Kuwait on Fire. Ground Troops?

> **🔥 KUWAIT — Iranian Drones Strike Mina Al-Ahmadi + Mina Abdullah Refineries**
>
> Iranian drones struck Kuwait's two largest refineries during Eid al-Fitr: Mina Al-Ahmadi and Mina Abdullah with a combined capacity of 730K bpd. Both facilities are offline. No timeline for restart has been given. This is the largest single oil supply disruption event of the conflict and the first direct strike on a GCC member state's refining infrastructure.

> **🇮🇶 IRAQ — Force Majeure Declared on All Foreign-Operated Oilfields**
>
> Iraq declared force majeure on all foreign-operated oilfields, citing its inability to ship crude through the Strait of Hormuz. Over 1 million bpd of Basra-grade crude is at risk of going offline if the blockade continues. Iraq's declaration signals that the Hormuz disruption has crossed from a shipping problem into a full-blown supply emergency.

> **⚠️ PENTAGON — Preparations for US Ground Troops into Iran**
>
> Pentagon officials reportedly made preparations for the possibility of US ground troops being deployed into Iran. This is the single most consequential geopolitical escalation signal of the conflict. Ground troops into Iran would trigger a multi-front war with regional spillover across every Gulf state and Hezbollah's full activation. The market dropped through the floor on this headline.

## War Signal Tracker | Day 20

| Signal | Detail | Mkt Read |
|---|---|---|
| **Kuwait refineries struck** | 730K bpd offline; Mina Al-Ahmadi + Mina Abdullah | **BEAR** |
| **Iraq force majeure** | 1M+ bpd Basra crude at risk; Hormuz blockade now structural | **BEAR** |
| **Pentagon ground troop prep** | Ground troops into Iran = full regional war scenario | **BEAR** |
| **Brent $112** | Highest since 2022; 5th consecutive weekly gain | **BEAR** |
| **COHR +15.4%, LITE +12%** | AI infrastructure cycle immune to war macro | **BULL** |
| **FDX beat + raised** | Domestic resilience holds; international = warning | **NEUTRAL** |
| **MU +8% still holding** | Memory demand not cracking despite macro | **BULL** |
| **S&P inclusion March 23** | COHR + LITE + VRT joining index; mechanical demand | **BULL** |

## Today's Event Calendar | Friday, March 20

| Time (ET) | Event | Priority |
|---|---|---|
| 9:45 AM | **S&P Global Flash PMIs (Mar) — first March activity read post-FOMC** | **HIGH** |
| All Day | **Quadruple Witching — equity options, index options, futures, single-stock futures expiry** | **HIGH** |
| All Day | **Iran / Kuwait / Iraq escalation — ground troop prep, IRGC response** | **HIGH** |
| All Day | **S&P 500 rebalance — COHR, LITE, VRT added Monday March 23** | **MED** |

## Trade Ideas | End-of-Week Positioning

> **▲ CONCENTRATE AI (MU, NVDA, COHR, LITE, SMCI) — This Sector Is Immune**
>
> Micron +8% from a historic beat. COHR +15.4%, LITE +12% on OFC 2026 + NVDA partnerships + S&P 500 inclusion March 23. The AI infrastructure buildout is genuinely immune to the war — hyperscalers are not cancelling data centres because Brent is at $112. COHR and LITE get mechanical passive buying Monday as they join the S&P 500. Buy the photonics dip on any Friday volatility.
>
> **Risk:** A surprise ceasefire would briefly hurt even AI names on rotation. Use as a buying opportunity.

> **▲ MAX ENERGY — XOM, CVX, SLB, XOP**
>
> Kuwait refineries offline. Iraq force majeure. Pentagon preparing ground troops. Brent $112 and going higher. Goldman says oil may stay in triple digits for years. This is the only sector with positive earnings revisions. Concentrate. Trim only on verified Hormuz reopening.
>
> **Risk:** Sudden diplomatic breakthrough — the only scenario that reverses energy.

> **▲ GOLD + DEFENCE BASKET — The War Continues**
>
> Gold at $3,305 and going higher. LMT, RTX, GD, NOC all benefiting from multi-year procurement tailwinds. Pentagon ground troop preparations confirm defence spending is accelerating. This basket is now the portfolio anchor.
>
> **Risk:** Peace deal — the only risk to both simultaneously.

> **▼ SELL EVERYTHING ELSE**
>
> Airlines: uninvestable. Consumer discretionary: cracking. High-multiple SaaS: rate + growth double compression. Small caps: in correction. EM-exposed: FX headwinds. This is not a market where diversification works. It's a market where conviction wins.

## Bottom Line | Two Markets. One War. Choose Your Side.

This is the most bifurcated market since the 2020 pandemic. One half is collapsing: the S&P is down 9.1% YTD, the Nasdaq is in correction, the Russell 2000 is in correction, airlines are uninvestable, consumer discretionary is cracking, and the Pentagon is preparing ground troops into Iran. Oil is at $112 and rising. The other half is exploding: Micron just posted the best memory earnings in history and guided to $33.5B. Coherent is up 15.4% in a single session. Lumentum is up 12%. Both join the S&P 500 on Monday. NVIDIA's $1T order book is validated by every earnings report in the AI supply chain. FedEx beat and raised. The AI infrastructure buildout is genuinely immune to the war — hyperscalers are not cancelling data centres because Brent is at $112. If anything, the AI demand is accelerating as compute becomes more strategic in a world of conflict. The playbook: concentrate into AI (MU, NVDA, COHR, LITE, SMCI), energy (XOM, CVX, SLB), gold (GLD), and defence (LMT, RTX, GD). Sell everything else. This is not a market where diversification works. It's a market where conviction wins.

---

DISCLAIMER: For internal research purposes only. Market data from Bloomberg, CNBC, Yahoo Finance, TradingEconomics, TheStreet as of Fri Mar 20, 2026 06:45 ET. War updates from Al Jazeera, CNN, Reuters, NPR, CBS News, CNBC, FDD Long War Journal. COHR/LITE data from StockTitan, Seeking Alpha, 24/7 Wall St, IndexBox. MU from Micron IR, CNBC. FDX from FreightWaves, CNBC, Investing.com. Oil from CNBC, OilPrice.com. OFC 2026 from StockTitan.
